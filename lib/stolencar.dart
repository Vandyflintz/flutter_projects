...
void main() => runApp(StolenCar());

class StolenCar extends StatelessWidget {
  @override
...
class MainStolenCar extends StatefulWidget {
  MainStolenCar({Key? key}) : super(key: key);
  @override
  State createState() => new StolenCarState();
}

class StolenCarState extends State<MainStolenCar>
...
return ntcMatch[1];
  ...
  TextEditingController _vincontroller = new TextEditingController();
  @override
  void dispose() {
    _animcon.dispose();
  ...
  Future<void> _salesdateselectDate(BuildContext context) async {
    ...
    if (newSelectedDate != null && newSelectedDate != _salesdateselectedDate)
      setState(() {
        ...
        _vincontroller.text = formatteddate.toString();
        setState(() {
  ...
  _fetchstolencar(String vintext) async {
    var url = "http://www.emkapp.com/emkapp/stolencar.php";
    var bdata = {"vinnum": vintext};
    ...
    try {
      ...
      if (jsonDecode(response.data).toString().contains("no data found")) {
        setState(() {
          _diagvisibility = false;
          pictureList = [];
          chassisno = "";
        ...
        var dbody = json.decode(response.data);

        print("Chassis no : " + dbody[0]["chassisno"]);
        setState(() {
          _diagvisibility = false;
          pictureList = dbody[0]["carimages"].split(",");
          carname = dbody[0]["carname"];
          year = dbody[0]["year"];
          make = dbody[0]["make"];
          model = dbody[0]["model"];
          color = evalJS(dbody[0]["color"]);
          madein = dbody[0]["madein"];
          carimages = dbody[0]["carimages"];
          abs = dbody[0]["abs"];
          wbl = dbody[0]["wbl"];
          transmission = dbody[0]["transmission"];
          mileage = dbody[0]["mileage"];
          drivetrain = dbody[0]["drivetrain"];
          dimensions = dbody[0]["dimensions"];
          chassisno = dbody[0]["chassisno"];
          engine = dbody[0]["engine"];
          lotnumber = dbody[0]["lotnumber"];
          trim = dbody[0]["trim"];
          body = dbody[0]["body"];
          shippingcost = dbody[0]["shippingcost"];
          towingcost = dbody[0]["towingcost"];
          purchasingcost = dbody[0]["purchasingcost"];
          carpartscost = dbody[0]["carpartscost"];
          buyername = dbody[0]["buyername"];
          buyercontact = dbody[0]["buyerphone"];
          buyeraddress = dbody[0]["buyeraddress"];
        });
        _stolencarWidget();
      }
          ...
          children: <Widget>[
            ...
            ClipRRect(
              ...
              child: Container(
                ...
                width: MediaQuery.of(this.context).size.width,
                height: MediaQuery.of(this.context).size.height - 236,
                child: Container(
                  width: MediaQuery.of(this.context).size.width,
                  height: MediaQuery.of(this.context).size.height,
