...
void main() => runApp(ActiveChannelAdmins());

class ActiveChannelAdmins extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    ...
    return new MaterialApp(
      ...
      home: MainActiveChannelAdmins(),
    );
  }
}

class MainActiveChannelAdmins extends StatefulWidget with PreferredSizeWidget {
  @override
  State createState() => new ActiveChannelAdminsState();

  @override
  Size get preferredSize => Size.fromHeight(100);
}

class ActiveChannelAdminsState extends State<MainActiveChannelAdmins>
  ...
  Widget _appBarTitle = new Text('Search for channel admins here');
  ...
  getactiveadminrecords() async {
    var url = "http://www.emkapp.com/emkapp/cadmins.php";
    ...
    try {
      ...
      if (jsonDecode(response.data)
        ...
        setState(() {
          ...
          _future = fetchActiveChannelAdmins();
  ...
  Future<List<CWorkers>> fetchActiveChannelAdmins() async {
    return activeadminresponse.map((e) => new CWorkers.fromJson(e)).toList();
  }

  Future<List<CWorkers>> sortActiveChannelAdmins() async {
    return secondactiveadminresponse
        .map((e) => new CWorkers.fromJson(e))
        .toList();
  }

  Future<List<CWorkers>> secondsortActiveChannelAdmins() async {
    return searchactiveadminresponse
        .map((e) => new CWorkers.fromJson(e))
        .toList();
  }

  Future<List<CWorkers>> searchActiveChannelAdmins() async {
    return searchactiveadminresponse
        .map((e) => new CWorkers.fromJson(e))
        .toList();
  }
  ...
  clearsearch() {
    ...
    setState(() {
      _future = sortActiveChannelAdmins();
      _hasSearched = false;
      _btnvisibility = false;
  ...
  _performsearch(String searchval) {
    ...
    if (searchactiveadminresponse.isEmpty) {
      ...
      setState(() {
        _diagvisibility = false;
        _emptyconvisibility = false;
        _future = searchActiveChannelAdmins();
    ...
    if (smval.contains("ascending")) {
      if (_hasSearched.toString() == "true" &&
        ...
        setState(() {
          _diagvisibility = false;
          _emptyconvisibility = false;
          _future = secondsortActiveChannelAdmins();
          listname = searchactiveadminresponse;
        });
      } else {
  ...
  _deletefromserver(String uid) async {
    ...
    try {
      ...
      if (jsonDecode(response.data).toString().contains("operation failed")) {
        ...
        _showsnackbar(
            "Worker with id : " + uid + " has been removed!", "Close");
  ...
  void _searchPressed() {
    setState(() {
      if (this._searchIcon.icon == Icons.search) {
        ...
        this._appBarTitle = new Text('Search for channel admins here');
      }
    });
  }
