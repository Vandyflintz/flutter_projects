...
void main() => runApp(ActiveAdmins());

class ActiveAdmins extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    ...
    return new MaterialApp(
      ...
      home: MainActiveAdmins(),
    );
  }
}

class MainActiveAdmins extends StatefulWidget with PreferredSizeWidget {
  @override
  State createState() => new ActiveAdminsState();

  @override
  Size get preferredSize => Size.fromHeight(100);
}

class ActiveAdminsState extends State<MainActiveAdmins>
  ...
  Widget _appBarTitle = new Text('Search for admins here');
  ...
  Future<List<WAdmins>>? _future;
  String _results = "";
  FutureBuilder<List<WAdmins>>? myfutureBuilder;
  @override
  void initState() {
    getuserconfig();
    getactiveadminrecords();
  ...
  getactiveadminrecords() async {
    var url = 'http://www.emkapp.com/emkapp/admins.php';
    setState(() {
      _diagvisibility = true;
    });
    var bdata = {"getcurrent": ""};
  ...
  Future<List<WAdmins>> fetchactiveadmins() async {
    return activeadminresponse.map((e) => new WAdmins.fromJson(e)).toList();
  }

  Future<List<WAdmins>> sortactiveadmins() async {
    return secondactiveadminresponse
        .map((e) => new WAdmins.fromJson(e))
        .toList();
  }

  Future<List<WAdmins>> secondsortactiveadmins() async {
    return searchactiveadminresponse
        .map((e) => new WAdmins.fromJson(e))
        .toList();
  }

  Future<List<WAdmins>> searchactiveadmins() async {
    return searchactiveadminresponse
        .map((e) => new WAdmins.fromJson(e))
        .toList();
  }
    ...
    if (smval.contains("ascending")) {
      if (_hasSearched.toString() == "true" &&
        ...
        setState(() {
          _diagvisibility = false;
          _emptyconvisibility = false;
          _future = secondsortactiveadmins();
          listname = searchactiveadminresponse;
        });
      } else {
  ...
  _deletefromserver(String uid) async {
    setState(() {
      _diagvisibility = true;
    });
    ...
    try {
      ...
      if (jsonDecode(response.data).toString().contains("operation failed")) {
        ...
        if (searchactiveadminresponse.isNotEmpty) {
          searchactiveadminresponse.removeWhere((el) => el["id"] == uid);
        }

        activeadminresponse.removeWhere((el) => el["id"] == uid);
        secondactiveadminresponse.removeWhere((el) => el["id"] == uid);
        ...
        _showsnackbar("Admin with id : " + uid + " has been removed!", "Close");
        if (_hasSearched.toString() == "false") {
          _refreshpage();
        } else if (_hasSearched.toString() == "true" &&
            searchactiveadminresponse.isEmpty) {
          setState(() {
            _diagvisibility = false;
            _emptyconvisibility = true;
  ...
  _refreshpage() async {
    ...
    try {
      var response = await dio.post(url, data: {"getcurrent": ""});
      print(response.data);
      searchactiveadminresponse.clear();
  ...
  _buildDataWidget() {
    ...
    if (activeadminresponse.isEmpty) {
      ...
      datawidget = SingleChildScrollView(
          ...
          child: Column(
            children: [
              FutureBuilder<List<WAdmins>>(
                  future: _future,
                  builder: (BuildContext context,
                      AsyncSnapshot<List<WAdmins>> snapshot) {
                    ...
                    if (snapshot.hasData) {
                      newsListSliver = ListView.builder(
                        ...
                        itemBuilder: (BuildContext context, int index) {
                          WAdmins item = snapshot.data![index];
  ...
  void _searchPressed() {
    setState(() {
      if (this._searchIcon.icon == Icons.search) {
        ...
        this._appBarTitle = new Text('Search for admins here');
      }
    });
  }
