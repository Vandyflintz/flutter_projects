...
import 'package:emkapp/allcarscontainer.dart';
...
void main() => runApp(UnShippedCars());

class UnShippedCars extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    ...
    return new MaterialApp(
      ...
      home: MainUnShippedCars(),
    );
  }
}

class MainUnShippedCars extends StatefulWidget with PreferredSizeWidget {
  @override
  State createState() => new UnShippedCarsState();

  @override
  Size get preferredSize => Size.fromHeight(100);
}

class UnShippedCarsState extends State<MainUnShippedCars>
    with TickerProviderStateMixin {
  ScrollController _scrollController = ScrollController(keepScrollOffset: true);
  late AnimationController _iconanimcontroller;
  late Animation<double> _iconanim;
  late ProgressDialog pr;
  late AnimationController _animcon, _fabcon;
  ...
  List carresponse = [];
  MainAllCars allCars = new MainAllCars();
  ...
  Future<List<CarsUnshipped>>? _future;
  String _results = "";
  FutureBuilder<List<CarsUnshipped>>? myfutureBuilder;
  bool _datevisibility = true, _isfavVisible = true;
  @override
  void initState() {
    getuserconfig();
    getcarrecords();
...
return ntcMatch[1];
  ...
  getcarrecords() async {
    ...
    FormData fdata = FormData.fromMap({"request": "unshipped"});
    var bdata = {"request": "unshipped"};
    setState(() {
    ...
    try {
      ...
      print(response.data);
      if (response.data.toString().contains("No data available for display")) {
        setState(() {
          _diagvisibility = false;
          _btnvisibility = false;
          _searchvisibility = false;
        ...
        setState(() {
          ...
          _future = fetchunshippedcars();
          listname = carresponse;
        });
        // _buildDataWidget();
  ...
  Future<List<CarsUnshipped>> fetchunshippedcars() async {
    return carresponse.map((e) => new CarsUnshipped.fromJson(e)).toList();
  }

  Future<List<CarsUnshipped>> sortunshippedcars() async {
    return secondcarresponse.map((e) => new CarsUnshipped.fromJson(e)).toList();
  }

  Future<List<CarsUnshipped>> secondsortunshippedcars() async {
    return searchcarresponse.map((e) => new CarsUnshipped.fromJson(e)).toList();
  }

  Future<List<CarsUnshipped>> searchunshippedcars() async {
    return searchcarresponse.map((e) => new CarsUnshipped.fromJson(e)).toList();
  }
  ...
  _performsearch(String searchval) {
    ...
    if (searchcarresponse.isEmpty) {
      ...
      setState(() {
        _diagvisibility = false;
        _emptyconvisibility = false;
        _future = searchunshippedcars();
        // listname = searchcarresponse;
      });
    }
  }
  ...
  _performsort(String sfval, String smval) {
    ...
    if (smval.contains("ascending")) {
      if (_hasSearched.toString() == "true" && searchcarresponse.isNotEmpty) {
        searchcarresponse
            .sort((a, b) => a[sfval].toString().compareTo(b[sfval].toString()));
        setState(() {
          _diagvisibility = false;
          _emptyconvisibility = false;
          _future = secondsortunshippedcars();
          listname = searchcarresponse;
        });
      } else {
        secondcarresponse
  ...
  PreferredSizeWidget _nbuildBar(BuildContext context) {
          ...
          Table(
            ...
            children: [
              ...
              TableRow(children: [
                Padding(
                  padding: const EdgeInsets.all(5.0),
                  child: Text("Shipper's Email Address",
                      style: TextStyle(
                        color: Color.fromRGBO(255, 255, 255, 1),
                      )),
                ),
                Padding(
                  padding: const EdgeInsets.all(5.0),
                  child: Text(shipperemailaddress,
                      style: TextStyle(
                        color: Color.fromRGBO(255, 255, 255, 1),
                      )),
                ),
              ...
              TableRow(children: [
                Padding(
                  padding: const EdgeInsets.all(5.0),
                  child: Text("Shipper's Residential Address",
                      style: TextStyle(
                        color: Color.fromRGBO(255, 255, 255, 1),
                      )),
                ),
                Padding(
                  padding: const EdgeInsets.all(5.0),
                  child: Text(shipperresidentialaddress,
                      style: TextStyle(
                        color: Color.fromRGBO(255, 255, 255, 1),
                      )),
                ),
          ...
          child: Container(
            ...
            decoration: new BoxDecoration(
              ...
              image: new CachedNetworkImageProvider(
                  "http://www.emkapp.com/emkapp/carsimg/" + image),
              fit: BoxFit.fill,
            )),
          ),
        ),
  ...
  _deletefromserver(String cno) async {
    var url = "http://www.emkapp.com/emkapp/allcars.php";
    var bdata = {"request": "deleteadmincar", "chassisno": cno};
    ...
    try {
      var response = await dio.post(url, data: fdata);
      print(response.data);
      if (jsonDecode(response.data).toString().contains("operation failed")) {
        setState(() {
          _diagvisibility = false;
          _showsnackbar("An error occured, please try again later.", "Close");
        });
      } else {
        if (searchcarresponse.isNotEmpty) {
          searchcarresponse.removeWhere((el) => el["chassisno"] == cno);
        }

        carresponse.removeWhere((el) => el["chassisno"] == cno);
        secondcarresponse.removeWhere((el) => el["chassisno"] == cno);
        ...
        _showsnackbar(
            "Car with chassis number : " + cno + " has been removed!", "Close");
        if (_hasSearched.toString() == "false") {
          _refreshpage();
        } else if (_hasSearched.toString() == "true" &&
            searchcarresponse.isEmpty) {
          setState(() {
            _diagvisibility = false;
            _emptyconvisibility = true;
            _results = "No search data is available for display";
            _btnvisibility = true;
            _searchvisibility = false;
          });
        }
  ...
  _carWidget() {
    ...
    if (carresponse.isEmpty) {
      ...
      datawidget = SingleChildScrollView(
          ...
          child: Column(
            children: [
              FutureBuilder<List<CarsUnshipped>>(
                  future: _future,
                  builder: (BuildContext context,
                      AsyncSnapshot<List<CarsUnshipped>> snapshot) {
                    ...
                    if (snapshot.hasData) {
                      newsListSliver = ListView.builder(
                          ...
                          itemBuilder: (BuildContext context, int index) {
                            CarsUnshipped item = snapshot.data![index];
  ...
  Widget build(BuildContext context) {
    ...
    return new Scaffold(resizeToAvoidBottomInset: false,
      appBar: boolTrue ? _nbuildBar(context) : null,
  ...
  _deleteadmincar(String dchassisno) async {
    var url = "http://www.emkapp.com/emkapp/allcars.php";
    var bdata = {"request": "deleteadmincar"};
    FormData fdata = FormData.fromMap(bdata);
    setState(() {
      _diagvisibility = true;
    });
  ...
  void _popupunsolddetails(
      ...
      String pshippername,
      String pshippercontact,
      String pshipperemailaddress,
      String pshipperresidentialaddress,
      String pcaraddress) {
    setState(() {
      ...
      shippername = pshippername;
      shippercontact = pshippercontact;
      shipperemailaddress = shipperemailaddress;
      shipperresidentialaddress = pshipperresidentialaddress;
      caraddress = pcaraddress;
